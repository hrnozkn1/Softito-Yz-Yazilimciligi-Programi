...
Model     : LSTM + Additive Attention
Fark      : Attention, LSTM'in tüm çıktılarına bakar -> bilgi kaybını azaltır
"""
...
def load_data():
    ...
    if not os.path.exists(path):
        ...
        print("    2. load_dataset('fancyzhx/ag_news', split='train').to_pandas().to_csv('data/ag_news.csv')")
...
class Attention(nn.Module):
    ...
    def forward(self, lstm_outputs):
        # lstm_outputs: (batch, seq_len, hidden_size * 2)
        attn_energies = self.v(torch.tanh(self.attn(lstm_outputs)))  # (batch, seq_len, 1)
        attn_weights = F.softmax(attn_energies.squeeze(-1), dim=1)  # (batch, seq_len)
        context = torch.bmm(attn_weights.unsqueeze(1), lstm_outputs)  # (batch, 1, hidden*2)
        return context.squeeze(1), attn_weights  # (batch, hidden*2), (batch, seq_len)

class LSTMAttentionClassifier(nn.Module):
    def __init__(self, vocab_size, embed_dim, hidden_size, num_layers, num_classes):
        super().__init__()
        self.embedding = nn.Embedding(vocab_size, embed_dim, padding_idx=0)
        self.lstm = nn.LSTM(embed_dim, hidden_size, num_layers, batch_first=True, dropout=0.3 if num_layers>1 else 0, bidirectional=True)
...
model = LSTMAttentionClassifier(VOCAB_SIZE, EMBED_DIM, HIDDEN_SIZE, NUM_LAYERS, NUM_CLASSES).to(DEVICE)
...
plt.title("Confusion Matrix — LSTM + Attention"); plt.ylabel("Gerçek"); plt.xlabel("Tahmin")
plt.tight_layout()
...
if len(all_attn_weights) > 0:
    ...
    plt.title("Attention Ağırlıkları (ilk örnek)"); plt.xlabel("Token Pozisyonu"); plt.ylabel("Ağırlık")
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURES_DIR, "04_attention_weights.png"), dpi=150, bbox_inches="tight")
    plt.close(); print("-> figures/04_attention_weights.png")
